...
Eventually I discovered that using `%0a` (newline) allows a bypass of the filter. `category1=a%0a%1` returns an error string `Invalid query parameters: invalid %-encoding (a%0a%1)`, which is a Ruby error.
