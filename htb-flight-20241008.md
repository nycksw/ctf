...
Steps for attacking this target:

- Fuzz subdomains to discover the `school.flight.htb` virtual host.
- Use LFI vulnerability in `index.php?view=` to recover the hash for user `svc_apache`.
- Crack the hash and spray the password to get valid credentials for user `s.moon`.
- Use `WRITE` permission on the `Shared` SMB share to upload malicious file(s) to get the hash for user `c.bum`.
- Use `RunAsCs.exe` to pivot from `s.moon` to `c.bum`.
- Upload a `aspx` webshell to the IIS development site bound to `127.0.0.1`.
- Catch the `aspx` shell to become `defaultapppool`.
- Use `GodPotato-NET4.exe` to elevate privileges to the system user with privileges as the machine account, `G0$`.
- Abuse `DCSync` to fetch hashes for `administrator`, `krbtgt`, etc. for full domain compromise.
